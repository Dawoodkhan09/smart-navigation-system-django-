...
class QrPreviewAdminMixin:
    ...
    campus.services.qr function of (request, obj) -> url.

    Shared by CampusAdmin (each campus's own QR - scanning it locks a
    visitor's session to that campus, see campus.views.campus_qr_landing)
...
class CampusAdmin(QrPreviewAdminMixin, admin.ModelAdmin):
    ...
    QrPreviewAdminMixin) - scanning it is how a visitor's phone locks
    into that campus's map, with no way to browse another campus's data
    afterwards (see campus.views.campus_qr_landing /
    api_views.resolve_default_campus).
    """
